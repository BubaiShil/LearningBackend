...
export const useApplicantsStore = create((set) => ({
  ...
  getApplicants: async (applicantId) => {
    try {
      const res = await axiosInstance.get(
        `/application/${applicantId}/applicants`
      );
  ...
  setStatus: async (statusId, status) => {
    try {
      const res = await axiosInstance.post(
        `/application/update-status/${statusId}`,
        { status }
      );
      console.log(res.data);
  ...
  getAppliedJob: async () => {
    try {
      ...
      set({ appliedJobs: res.data.application });
    } catch (error) {
      console.error("getappliedjobs error:", error);
      toast.error(
        error.response?.data?.message || "getting appliedJobs failed"
      );
    }
  },
}));
